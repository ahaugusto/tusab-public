# Brain'IAC — Contexto de Projeto para Claude Code
...
Brain'IAC é um sistema de gestão de conhecimento pessoal (PKM) com IA local.
...
- `BRAINIAC_DATA_DIR` env var define onde os dados ficam (usado em testes e no Electron packaged)
...
```
api_brainiac.py          ← thin entry point (165 linhas); monta app FastAPI + routers
  │
  ├── brainiac_engine/api/         ← routers FastAPI por domínio
  ...
  │
  ├── brainiac_engine/motor/       ← extração e Drive
  │     drive.py                   OAuth2 Google Drive + upload (get_drive_service, upload_txt_como_gdoc_seguro)
  │     extraction.py              engine principal (brainiac_engine), utilitários, relatórios
  │
  ├── brainiac_engine/agent/       ← RAG local
  ...
  │
  ├── brainiac_engine/state.py     ← AppState singleton + LogRedirector
  └── brainiac_engine/storage.py   ← paths de dados + IO atômico
```
...
`motor_brainiac.py` e `agent_brainiac.py` são re-exports puros — Electron e código legado
...
**Override:** `BRAINIAC_DATA_DIR` env var substitui o root de dados (usado em testes e Electron packaged).

---

### `brainiac_engine/state.py`
...
**Importante:** importar `state.py` APÓS `motor_brainiac` e `agent_brainiac` para preservar a ordem original de inicialização.

---

### `brainiac_engine/agent/config.py`
...
### `brainiac_engine/agent/index.py`
...
### `brainiac_engine/motor/drive.py`
...
### `brainiac_engine/motor/extraction.py`
...
**Entrypoints:** `brainiac_engine(canal_url, evento_pausa, evento_cancelar, fontes_filtro)` (função principal),
...
**Nota de naming:** a função `brainiac_engine()` tem o mesmo nome do pacote — sem conflito porque está em `motor/extraction.py`, não no `__init__.py` do pacote.

---

### `brainiac_engine/api/router_status.py`
...
### `brainiac_engine/api/router_extraction.py`
**Tags:** extração, canal, iniciar, pausar, cancelar, fontes, motor, YouTube
**Rotas:** `POST /set-channel`, `POST /start`, `POST /pause`, `POST /cancel`.
**Background:** `run_motor()` — chama `motor_brainiac.brainiac_engine()` em background task; usa `threading.Thread` para auto-reset de status após 15s.
**Validação:** `_YT_URL_RE` — regex que aceita `/@canal`, `/channel/...`, `/c/...`; rejeita qualquer outra coisa.

---

### `brainiac_engine/api/router_agent.py`
...
### `brainiac_engine/api/router_repositorio.py`
...
```
tests/
  conftest.py               fixture: BRAINIAC_DATA_DIR → tempdir antes de qualquer import
...
| `brainiac_engine/` (não `brainiac/`) | `brainiac.spec` existe na raiz (PyInstaller) — colisão de nome |
...
| Shims na raiz (`motor_brainiac.py`, `agent_brainiac.py`) | Electron `extraResources.filter` e testes importam pelo nome antigo — zero breaking change |
...
| Quero... | Vá para... |
|----------|------------|
| Mudar onde os dados ficam | `brainiac_engine/storage.py` |
| Mudar o estado global / adicionar flag | `brainiac_engine/state.py` → `AppState` |
| Mudar o fluxo de extração YouTube | `brainiac_engine/motor/extraction.py` → `brainiac_engine()` |
| Mudar integração com Drive | `brainiac_engine/motor/drive.py` |
| Mudar como o BM25 indexa | `brainiac_engine/agent/index.py` → `indexar()` |
| Mudar como o chat recupera contexto | `brainiac_engine/agent/chat.py` → `_recuperar_contexto()` |
| Adicionar rota de status/drive | `brainiac_engine/api/router_status.py` |
| Adicionar rota de extração | `brainiac_engine/api/router_extraction.py` |
| Adicionar rota de agente | `brainiac_engine/api/router_agent.py` |
| Adicionar rota de repositório | `brainiac_engine/api/router_repositorio.py` |
