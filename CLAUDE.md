# Sebayt — Contexto de Projeto para Claude Code
...
Sebayt é um sistema de gestão de conhecimento pessoal (PKM) com IA local.
...
- `SEBAYT_DATA_DIR` env var define onde os dados ficam (usado em testes e no Electron packaged)
...
```
api_sebayt.py          ← thin entry point (165 linhas); monta app FastAPI + routers
  │
  ├── sebayt_engine/api/         ← routers FastAPI por domínio
  ...
  │
  ├── sebayt_engine/motor/       ← extração e Drive
  │     drive.py                   OAuth2 Google Drive + upload (get_drive_service, upload_txt_como_gdoc_seguro)
  │     extraction.py              engine principal (sebayt_engine), utilitários, relatórios
  │
  ├── sebayt_engine/agent/       ← RAG local
  ...
  │
  ├── sebayt_engine/state.py     ← AppState singleton + LogRedirector
  └── sebayt_engine/storage.py   ← paths de dados + IO atômico
```
...
`motor_sebayt.py` e `agent_sebayt.py` são re-exports puros — Electron e código legado
...
**Override:** `SEBAYT_DATA_DIR` env var substitui o root de dados (usado em testes e Electron packaged).

---

### `sebayt_engine/state.py`
...
**Importante:** importar `state.py` APÓS `motor_sebayt` e `agent_sebayt` para preservar a ordem original de inicialização.

---

### `sebayt_engine/agent/config.py`
...
### `sebayt_engine/motor/extraction.py`
...
**Entrypoints:** `sebayt_engine(canal_url, evento_pausa, evento_cancelar, fontes_filtro)` (função principal),
...
**Nota de naming:** a função `sebayt_engine()` tem o mesmo nome do pacote — sem conflito porque está em `motor/extraction.py`, não no `__init__.py` do pacote.

---

### `sebayt_engine/api/router_status.py`
...
### `sebayt_engine/api/router_repositorio.py`
...
```
tests/
  conftest.py               fixture: SEBAYT_DATA_DIR → tempdir antes de qualquer import
...
| `sebayt_engine/` (não `Sebayt/`) | `Sebayt.spec` existe na raiz (PyInstaller) — colisão de nome |
...
| Shims na raiz (`motor_sebayt.py`, `agent_sebayt.py`) | Electron `extraResources.filter` e testes importam pelo nome antigo — zero breaking change |
...
| Quero... | Vá para... |
|----------|------------|
| Mudar onde os dados ficam | `sebayt_engine/storage.py` |
| Mudar o estado global / adicionar flag | `sebayt_engine/state.py` → `AppState` |
| Mudar o fluxo de extração YouTube | `sebayt_engine/motor/extraction.py` → `sebayt_engine()` |
| Mudar integração com Drive | `sebayt_engine/motor/drive.py` |
| Mudar como o BM25 indexa | `sebayt_engine/agent/index.py` → `indexar()` |
| Mudar como o chat recupera contexto | `sebayt_engine/agent/chat.py` → `_recuperar_contexto()` |
| Adicionar rota de status/drive | `sebayt_engine/api/router_status.py` |
| Adicionar rota de extração | `sebayt_engine/api/router_extraction.py` |
| Adicionar rota de agente | `sebayt_engine/api/router_agent.py` |
| Adicionar rota de repositório | `sebayt_engine/api/router_repositorio.py` |
