...
| **FHIR** (`ResearchStudy` e outros recursos) | Padrão HL7 de interoperabilidade de dados clínicos — cada recurso (`Patient`, `Observation`, `ResearchStudy`) é JSON/XML tipado, mas tem um campo `text.div` (Narrative) obrigatório por spec: resumo em HTML feito para leitura humana | Mesmo público de ClinicalTrials.gov — `ResearchStudy` é o equivalente estruturado a um registro de ensaio clínico | Mais barato tecnicamente que ClinicalTrials.gov: não exige parsear o schema tipado inteiro, só extrair `text.div` + limpar HTML (mesmo padrão de `_detectar_formato_especial()` do WhatsApp/Zoom) e indexar como texto corrido — reaproveita o chunking BM25 atual. Avaliado em jul/2026, ver `agents/_historia.md` |
