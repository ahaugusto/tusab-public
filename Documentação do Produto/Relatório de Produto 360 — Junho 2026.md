# Relatório de Produto 360° — Brain'IAC
...
- **Leitura correta do ecossistema:** Gemma 4 e o avanço dos modelos locais são combustível, não ameaça — o Brain'IAC é camada de infraestrutura de conhecimento, não motor.
...
**Dimensão:** api_brainiac.py 1.281 linhas (31 endpoints) · motor_brainiac.py 923 linhas · agent_brainiac.py 809 linhas.
...
- `state.stats` — atualizado pelo LogRedirector (thread do motor) e lido pela API (api_brainiac.py:170–195);
- `_bm25_cache` — dois chats simultâneos no mesmo canal disparam dupla reconstrução do índice (agent_brainiac.py:70–74);
...
- **Servidor MCP é a aposta estratégica mais subestimada do roadmap:** transforma o Brain'IAC de app destino em infraestrutura que qualquer agente (Claude, Cursor, etc.) consulta — muda a categoria do produto e cria lock-in técnico real.
