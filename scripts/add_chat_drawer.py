# -*- coding: utf-8 -*-
"""Adds floating chat drawer to BrainIAc App.jsx"""
import re, sys

path = r'C:\Users\augus\Desktop\Brainiac\web_interface\src\App.jsx'
...
# ── 2. Remove Chat section from agent tab ─────────────────────────────────
chat_start = src.find('              {/* Chat */}')
if chat_start == -1:
    print("ERROR: chat section not found"); sys.exit(1)
section_end = src.find('</section>', chat_start) + len('</section>')
src = src[:chat_start] + src[section_end:]
print("Chat section removed")

# ── 3. Build drawer JSX (no f-string to avoid $ conflicts) ─────────────────
DRAWER = r"""
...
# ── 4. Insert drawer before scroll-to-top button ─────────────────────────
INSERT_BEFORE = '          {/* Botão voltar ao topo */'
if INSERT_BEFORE not in src:
    print("ERROR: insertion point not found"); sys.exit(1)

src = src.replace(INSERT_BEFORE, DRAWER + INSERT_BEFORE, 1)
print("Drawer inserted")
